#!/usr/bin/env node
/** TEST E2E COMPLETO — MÓDULO REPORTS */
...
async function main() {
  log(`${C.bold}TEST E2E — REPORTS${C.reset}`);
  ...
  const ENDPOINTS = [
    ...
    ["GET", "/reports/data/operational"],
    ["GET", "/reports/data/financial"],
    ["GET", "/reports/usage-stats"],
    ["GET", "/reports/definitions/test-uuid"],
    ["GET", "/reports/templates/test-uuid"],
    ["POST", "/reports/generate"],
  ];
  for (const [m, p] of ENDPOINTS) {
    await sleep(3000);
    const result = await http(m, `${API_URL}${p}`, m === "POST" ? { body: { templateId: "test", parameters: {} } } : {});
